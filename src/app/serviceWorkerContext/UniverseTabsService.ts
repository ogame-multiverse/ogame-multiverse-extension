...
export class UniverseTabsService {
  ...
  public async RefreshUniverseTabAsync(universeKey: string, options?: { openIfMissing?: boolean; openOnReloadTab?: boolean; ensureRefresh?: boolean }): Promise<boolean> {
    const normalizedUniverseKey = UniverseDataNormalizer.NormalizeUniverseKey(universeKey);
    if (!normalizedUniverseKey) return false;

    const chromeApi = (globalThis as { chrome?: typeof chrome }).chrome;
    if (!chromeApi?.tabs?.query || !chromeApi.tabs.reload) return false;

    const tabs = await chromeApi.tabs.query({ url: GlobalConstants.OGAME_URL_GAME_PATTERN });
    const matchingTabs = tabs.filter((tab: chrome.tabs.Tab) => this.ExtractUniverseKeyFromTab(tab) === normalizedUniverseKey && typeof tab.id === 'number');
    if (matchingTabs.length === 0) {
      if (!options?.openIfMissing && !options?.ensureRefresh) return false;
      if (!chromeApi.tabs.create) return false;

      const createdTab = await chromeApi.tabs.create({
        url: this.BuildUniverseOverviewUrl(normalizedUniverseKey),
        active: !!options?.openIfMissing || !!options?.openOnReloadTab,
      });
      return typeof createdTab.id === 'number';
    }

    const activeTab = matchingTabs.find((tab: { active: any; }) => tab.active);
    const tabToReload = activeTab || matchingTabs[0];
    if (typeof tabToReload.id !== 'number') return false;

    if (options?.openOnReloadTab && chromeApi.tabs.update) await chromeApi.tabs.update(tabToReload.id, { active: true, url: this.BuildUniverseOverviewUrl(normalizedUniverseKey), });

...
export class UniversePanelController {
  ...
  constructor(private readonly messaging: SidePanelSafeMessaging) { }
  ...
  public async RefreshAsync(): Promise<void> {
    ...
    const universeStatuses = await this.messaging.SendP2SWSafe<SidePanelUniverseStatus[]>('LIST_UNIVERSE_STATUSES', {}, []);
  ...
  private BuildUniverseRow(status: SidePanelUniverseStatus): UniverseRowView {
    ...
    const rowView: UniverseRowView = {
      ...
      updateWarningState: () => {
        const thresholdMinutes = status.WarningThresholdMinutes ?? this.defaultWarningThresholdMinutes;
        updateWarningState(thresholdMinutes);
      },
    };

    // Remplissage du select des seuils
    const selectedThreshold = status.WarningThresholdMinutes;
    ...
    thresholdSelect.addEventListener('change', () => {
      ...
      const normalizedKey = this.NormalizeUniverseKey(status.universeKey);

      status.WarningThresholdMinutes = selectedMinutes;

      this.saveManager.GetExtensionLocalDataAsync({ universeKey: normalizedKey }).then(async (localSave) => {
        localSave.WarningThresholdMinutes = selectedMinutes;
        await this.saveManager.SaveExtensionLocalDataAsync({ universeKey: normalizedKey, localSave });
      });
    ...
    refreshButton.addEventListener('click', async () => {
      ...
      try {
        await this.messaging.SendP2SWSafe<{ refreshed: boolean }>(
          'RELOAD_UNIVERSE_TAB',
          {
            universeKey: status.universeKey,
            openIfMissing: true,
            openOnReloadTab: true,
            ensureRefresh: true,
          },
          { refreshed: false }
        );
    ...
    removeButton.addEventListener('click', async () => {
      ...
      try {
        await this.messaging.SendP2SWSafe<{ removed: boolean }>(
          'REMOVE_UNIVERSE',
          {
            universeKey: status.universeKey
          }, { removed: false }
    ...
    displayOwnFleetCheckboxSetting.checked = status.ShowOwnFleetIndicator;
    updateBooleanRowAttribute('show-own-fleets-indicator', displayOwnFleetCheckboxSetting.checked);
    displayOwnFleetCheckboxSetting.addEventListener('change', () => {
      ...
      const normalizedKey = this.NormalizeUniverseKey(status.universeKey);
      this.saveManager.GetExtensionLocalDataAsync({ universeKey: normalizedKey }).then(async (localSave) => {
        localSave.ShowOwnFleetIndicator = checked;
        await this.saveManager.SaveExtensionLocalDataAsync({ universeKey: normalizedKey, localSave });
      });
    ...
    displayUnreadMailCheckboxSetting.checked = status.ShowUnreadMessagesIndicator;
    updateBooleanRowAttribute('show-unread-mail-indicator', displayUnreadMailCheckboxSetting.checked);
    displayUnreadMailCheckboxSetting.addEventListener('change', () => {
      const checked = displayUnreadMailCheckboxSetting.checked;


      const normalizedKey = this.NormalizeUniverseKey(status.universeKey);
      this.saveManager.GetExtensionLocalDataAsync({ universeKey: normalizedKey }).then(async (localSave) => {
        localSave.ShowUnreadMessagesIndicator = checked;
        await this.saveManager.SaveExtensionLocalDataAsync({ universeKey: normalizedKey, localSave });
      });
    ...
    displayUnreadChatCheckboxSetting.checked = status.ShowUnreadChatMessagesIndicator;
    updateBooleanRowAttribute('show-unread-chat-indicator', displayUnreadChatCheckboxSetting.checked);
    displayUnreadChatCheckboxSetting.addEventListener('change', () => {
      const checked = displayUnreadChatCheckboxSetting.checked;


      const normalizedKey = this.NormalizeUniverseKey(status.universeKey);
      this.saveManager.GetExtensionLocalDataAsync({ universeKey: normalizedKey }).then(async (localSave) => {
        localSave.ShowUnreadChatMessagesIndicator = checked;
        await this.saveManager.SaveExtensionLocalDataAsync({ universeKey: normalizedKey, localSave });
      });
  ...
  private UpdateUniverseRow(rowView: UniverseRowView, status: SidePanelUniverseStatus): void {
    ...
    updateCounter(rowView.unreadMessagesValue, status.newMessages);
    updateCounter(rowView.unreadChatMessagesValue, status.newChatMessages);
    updateCounter(rowView.hostileFleetCountValue, status.hostileFleetCount);
    updateCounter(rowView.friendlyFleetCountValue, status.friendlyFleetCount);
    updateCounter(rowView.ownFleetCountValue, status.ownFleetCount);
